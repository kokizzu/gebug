...
export default class ConfigService {
  private static url(relative: string): string {
    const port = import.meta.env.VUE_APP_PORT || 3030
  ...
  static decode(config: any): Config {
    return {
      ...
      exposePorts: config.exposePorts.map((p: any) => ({ port: p })),
      networks: config.networks.map((n: any) => ({ network: n })),
      environment: config.environment.filter((e: any) => e).map((e: string) => {
        const parts = e.split('=')
        if (parts.length > 1)
  ...
  static async get(): Promise<{ config?: Config; location?: string }> {
    const defaultValue: { config?: Config; location?: string } = {}
